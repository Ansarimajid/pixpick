...
class Box:
    ...
    def __post_init__(self):
        ...
        if not (0 <= self.x1 < self.x2 <= self.image_width):
            raise ValueError(f"x coords out of bounds: {self.x1}, {self.x2} (image width={self.image_width})")

        if not (0 <= self.y1 < self.y2 <= self.image_height):
            raise ValueError(f"y coords out of bounds: {self.y1}, {self.y2} (image height={self.image_height})")
    ...
    def as_numpy(self) -> np.ndarray:
        """Shape (4,) int array — [x1, y1, x2, y2]."""
        return np.array(self.xyxy, dtype=np.int32)

    # ------------------------------------------------------------------ #
    # Adapter shortcut (delegates to the YOLO adapter)                    #
    # ------------------------------------------------------------------ #

    def to_yolo(self) -> dict:
        from pixpick.adapters.yolo import YOLOAdapter
        return YOLOAdapter().convert(self)
    ...
    def load(cls, path: str | Path) -> "Box":
        """Reconstruct a Box from a saved JSON file."""
    ...
    def visualize(self, image: np.ndarray, color: tuple = (0, 255, 0), thickness: int = 2) -> np.ndarray:
        ...
        cv2.putText(canvas, label, (self.x1, max(self.y1 - 8, 12)),
